...
const LoginItems = (
  <>
    <Form.Item name="username" label="用户名" rules={[{ required: true, message: '姓名不能为空' }]}>
...
const LoginFooter = (changer: Function) => (
  <>
    <p className="tooltip">
      未注册？
      <span className="primary" onClick={() => changer('register')}>
        点击这里
      </span>
      注册账号
    </p>
  ...
  <>
    ...
    <Form.Item
      ...
      rules={[
        {
          validator: (_, v) =>
            form.getFieldValue('password') === v ? Promise.resolve() : Promise.reject('密码不一致！'),
        },
      ]}
    >
...
const RegisterFooter = (changer: Function) => (
  <>
    <p className="tooltip">
      注册过了？
      <span className="primary" onClick={() => changer('login')}>
        点击这里
      </span>
      登录账号
    </p>
